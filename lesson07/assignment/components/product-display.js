//first arguemnt is the component name, second is an object to configure our component
...
app.component('product-display', {
    ...
    // props: ['products'],
    template:
    //activates the VS Code extension es6-string-html, 
    /*html*/
        `       
     <div v-for="(product, index) in products" class="card" style="max-width: 18rem;">
                        <!--gets products img-->
    <img class="card-img-top" v-bind:src="product.imgSrc">
    <div></div>
    <div class="card-body">
                                     <!--gets product name-->
        <h5 class="card-title name">{{product.name}}</h5>
                                                                  <!--gets product price-->
        <p class="card-text price"><strong>Cost:</strong>{{product.price}}</p>
